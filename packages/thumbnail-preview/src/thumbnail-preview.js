...
/**
 * Represents a ThumbnailPreview component for the videojs player.
...
class ThumbnailPreview extends Component {
   ...
   *
   * This method serves as a proxy to the main `onCancel` handler, ensuring
   ...
   *
   * This method serves as a proxy to the main `onPlayerResize` handler, ensuring
   * that additional logic can be executed or making it easier to detach the
   * event listener later.
   *
   * @private
  ...
  /**
   * Creates an instance of ThumbnailPreview.
   *
  ...
  constructor(player, options = {}) {
    ...
    this.player().ready(() => this.init());
  }

  /**
   * Initializes the ThumbnailPreview component.
   *
   * This method sets up the component by verifying that the required
   * `ProgressControl` component is available in the player's `ControlBar`.
   *
   * If the `ProgressControl` is not found, this component hides itself, and
   * removes any event listeners to avoid unnecessary behavior.
   *
  ...
  initListeners() {
    ...
    ['mousemove', 'touchmove'].forEach(
      event => progressControl.on(event, this.#onMove)
    );

    ['mouseleave', 'touchcancel', 'touchend'].forEach(
      event => progressControl.on(event, this.#onCancel)
    );
  ...
  removeListeners() {
    ...
    if (progressControl) {
      ['mousemove', 'touchmove'].forEach(
        event => progressControl.off(event, this.#onMove)
      );

      ['mouseleave', 'touchcancel', 'touchend'].forEach(
        event => progressControl.off(event, this.#onCancel)
      );
    }
  ...
  onMove(event) {
    ...
    const point = ThumbnailPreview.calculatePointBoundary(
  ...
  calculateFrame(seekingTime) {
    ...
    const thumb = this.el().querySelector('.pbw-thumbnail');
  ...
  createEl(tag = 'div', props = {}, attributes = {}) {
    ...
    this.thumbnail = videojs.dom.createEl('img', { className: 'pbw-thumbnail' });
    this.thumbnail.src = this.options().sprite.url;
    el.appendChild(this.thumbnail);
...
ThumbnailPreview.prototype.options_ = {
  className: 'pbw-thumbnail-preview',
  displayThresholdHeight: 395,
