...
ThumbnailPreview is a custom Video.js component that adds interactive thumbnail previews to the
player's progress bar. When users hover through the timeline, the component displays
frame-accurate thumbnails extracted from a sprite sheet. It supports responsive scaling via
...
const player = videojs('my-player', { thumbnailPreview: true });
```
...
When initializing the `ThumbnailPreview` component, you can pass an `options` object to configure
its behavior:
