...
| `pillarboxPlaylistMenuDialog.itemComponentName`         | String  | `PillarboxPlaylistMenuItem` | Name of the Video.js Component used to render each playlist item. Allows replacing the default playlist item component. The specified component must be registered with Video.js via `videojs.registerComponent` before the dialog is created.              |
