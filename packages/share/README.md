...
| `url` | `string\|function` | Current page URL | Base URL used to generate share links; functions receive `(player, component, context)`. |
| `defaultOption` | `string` | `'episode'`, or `'currentTime'` when `includeCurrentTime` is enabled | Initially selected URL option. |

When `url` is a function, the `context` argument exposes available media
information without imposing any application-specific URL format:
